...
namespace ns3
{
  namespace lorawan
  {
    ...
    ClassAEndDeviceLorawanMac::FailedReception(Ptr<Packet const> packet)
    {
      ...
      NS_LOG_INFO("Switch to sleep after failed receiption");
      m_phy->GetObject<EndDeviceLoraPhy>()->SwitchToSleep();
    ...
    ClassAEndDeviceLorawanMac::TxFinished(Ptr<const Packet> packet)
    {
      ...
      if (GetMType () == LorawanMacHeader::CONFIRMED_DATA_UP){
      ...
      NS_LOG_INFO("Switch the PHY to sleep");
      m_phy->GetObject<EndDeviceLoraPhy>()->SwitchToSleep();
    }
    ...
    ClassAEndDeviceLorawanMac::CloseFirstReceiveWindow(void)
    {
      ...
      case EndDeviceLoraPhy::STANDBY:
        ...
        NS_LOG_INFO("Turn PHY layer to SLEEP as it is in STANDBY");
        phy->SwitchToSleep();
        break;
      }
    }
    ...
    */
    void ClassAEndDeviceLorawanMac::ForceDeviceSleep(void)
    {
        Ptr<EndDeviceLoraPhy> phy = m_phy->GetObject<EndDeviceLoraPhy>();
        phy->SwitchToSleep();
        NS_LOG_INFO("Forced device sleep");
    }
